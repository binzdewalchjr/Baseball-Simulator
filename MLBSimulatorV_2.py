...
PossibleTeams=["Angels","Astros","Athletics","Blue Jays","Braves","Brewers","Cardinals","Cubs","Diamondbacks","Dodgers"\
               ,"Giants","Indians","Mariners","Marlins","Mets","Nationals","Orioles","Padres","Pirates","Phillies"\
...
for i in range(0,2):
    ...
    for x in range(len(PlayersToPass)):
        if int(dataframep.iloc[x][15]) != 0:
            ...
            PitchCount = float(dataframep.iloc[x][35])#/ float(dataframep.iloc[x][8])  # Pitch count has holder values in the current form of the exel file.  Correct values need to be used
            ...
            p1 = Pitcher(dataframep.iloc[x][2], dataframep.iloc[x][1], dataframep.iloc[x][34], Strikeout, Walk, HomeRun, Hit, PitchCount, BattersFaced, GamesPitched, InningsPitched, AverageAgainst, GroundOverAir)
            if i == 0:
...
###Uncomment for manual entry of Away hitting   
...
for x in range(0,9):
    ...
    while flag == True:
        ...
        Player = input("Enter A player for spot " + lineupnumber + " in the Away lineup. To list type 'ls': ")
        if Player in list_of_batters:
            addtolineup = dictionary_of_batters[Player]
            AwayLineup.append(addtolineup)
            flag = False
        elif Player == "ls":
            for people in list_of_batters:
                print(people)
        else:
...
###Uncomment for manual entry of Away pitching
AwayBullpen = []
flag = True
while flag == True:
      Player = input("Enter starting pitcher. To list type 'ls':")
      if Player in list_of_pitchers:
          addtolineup = dictionary_of_pitchers[Player]
          AwayLineup.append(addtolineup)
          flag = False
      elif Player == "ls":
            for people in list_of_pitchers:
                print(people)
      else:
        print("Please enter a valid player's name: ")
# AwayBullpen.append(addtolineup)

###Uncomment for manual entry of bullpen
# for x in range(0,8):
#     flag = True
#     while flag == True:
#         lineupnumber = str(x + 1)
...
###Uncomment for manual entering of Home batters   
...
for x in range(0,9):
    ...
    while flag == True:
        lineupnumber = str(x+1)
        Player = input("Enter A player for spot " + lineupnumber + " in the Home lineup. To list type 'ls': ")
        if Player in list_of_batters1:
            addtolineup = dictionary_of_batters1[Player]
            HomeLineup.append(addtolineup)
            flag = False
        elif Player == "ls":
            for people in list_of_batters1:
                print(people)
        else:
            print("Please enter a valid player's name: ")
     #
...
###Uncomment for manual entering of Home pitching
#Prompting the user to input the Starting and Relief Pitchers for the home team 
...
while flag == True:
      ...
      if Player in list_of_pitchers1:
          ...
          flag = False
            ...
            for people in list_of_pitchers1:
                print(people)
      else:
        print("Please enter a valid player's name: ")
...
#############
###Uncomment section for auto fill of best players


###Uncomment to autofill batters
# HomeLineup = []
# for x in range (0,9):
...
HomeBullpen = []
for x in range (0,9):
    if Pitchers_list1[x] not in HomeLineup:
...
AwayBullpen = []
for x in range (0,9):
    if Pitchers_list[x] not in AwayLineup:
        AwayBullpen.append(Pitchers_list[x])

###Sets substitution unavailibility lists
AwayUsedPlayers = []
for player in AwayLineup:
    AwayUsedPlayers.append(player.name)
    
HomeUsedPlayers = []
for player in HomeLineup:
    HomeUsedPlayers.append(player.name)
...
def AtBat(Pitcher,Batter,Bases,Outs,Runners, Score):
    ...
    H = Batter.H * (Pitcher.AA)/ 0.248 
    BB = Batter.BB * Pitcher.BB / 0.090 
    HBP = Batter.HBP
...
def HalfInning(TeamHitting, LineUp, SpotInLineup, Pitcher, Score, Hits, PitchCount, GameMode):
    ...
    while Outs < 3:
        ...
        if TeamHitting == "Away":
            AwayScore += Runs
            Score[0] = AwayScore
            PitchCount[0] += (Pitcher.NP/Pitcher.TBF)
        ...
        if GameMode == "AtBat":
            ...
            AskUser = input("To continue type 'y' , to change pitcher type 'p' to change batter type 'b', to simulate to the end of the game type 'e', to quit type 'q'\n")
            if AskUser == "y":
                continue
            if AskUser == "p":
                if TeamHitting == "Away":
            ...
            if AskUser == "b":
                ...
                if TeamHitting == "Away":
                    ...
                    AwayLineup[SpotInLineupINN] = FindBatter
                    AwayUsedPlayers.append(NewBatter)
                else:
                    FindBatter = dictionary_of_batters1[NewBatter]
                    HomeLineup[SpotInLineupINN] = FindBatter
                    HomeUsedPlayers.append(NewBatter)
            if AskUser == "e":
                GameMode = "100Game"
                #GameNumber = 99
            if AskUser == "q":
...
##Main Game Code
def MainGame(AwayTeamWins, HomeTeamWins, GameMode):
    CurrentInning = 1
    TeamHitting = "Away"
    LineUp = AwayLineup
    Pitcher = HomePitcher
    ...
    while CurrentInning < 10 or (CurrentInning >= 10 and (Score[0] == Score[1] and InningStatus == "Top")) or (CurrentInning >=10 and (Score[0]>=Score[1] and InningStatus == "Bottom")) or (CurrentInning == 9.5 and Score[1]< Score[0]):
    #(CurrentInning >= 10 and Score[0] == Score[1]) or (CurrentInning == 9.5 and Score[0]< Score[1]):
        ...
        if GameMode == "100Game":
            ...
            if CurrentInning >= 10:
                ...
                if TeamHitting == "Away":
                    print("\n###\nTop of the {0}\n{1} {2}\n{3} {4}\n".format(int(CurrentInning//1),AwayTeam,Score[0],HomeTeam,Score[1]))
                    print("{0} is on the mound\n###".format(HomeLineup[9]))
                if TeamHitting == "Home":
                    print("\n###\nBottom of the {0}\n{1} {2}\n{3} {4}\n".format(int(CurrentInning//1),AwayTeam,Score[0],HomeTeam,Score[1]))
                    print("{0} is on the mound\n###".format(AwayLineup[9]))
        ...
        if GameMode == "Inning" or GameMode == "AtBat":
            if TeamHitting == 'Away':
                ...
                TempSpot = SpotInLineup[0]+1
                if SpotInLineup[0] == 7:
                    OnDeck = LineUp[SpotInLineup[0]+1]
                    TempSpot2 = TempSpot+1
                    InTheHole = LineUp[0]
                    Tempspot3 = 1
                    ...
                    TempSpot3 = TempSpot + 2
                
                InningStatus = 'Top'
            if TeamHitting == 'Home':
                DueUp = LineUp[SpotInLineup[1]]
                TempSpot = SpotInLineup[1] +1
                if SpotInLineup[1] == 7:
            ...
            print("Due up:\n{0}) {1}\n{2}) {3}\n{4}) {5}\n".format(TempSpot,DueUp, TempSpot2,OnDeck, TempSpot3,InTheHole))
            ...
            if AskUser == "b":
                ...
                if NewBatter not in change_batters_list or NewBatter in TempLineup:
                    while NewBatter not in change_batters_list:
                        ...
                        if NewBatter == "q":
                            sys.exit()
                LineUpSwap = int(input("Please enter line up spot to place batter\n"))-1
                
                if TeamHitting == "Away":
                    FindBatter = dictionary_of_batters[NewBatter]
                    AwayLineup[LineUpSwap] = FindBatter
                    AwayUsedPlayers.append(NewBatter)
                else:
                    FindBatter = dictionary_of_batters1[NewBatter]
                    HomeLineup[LineUpSwap] = FindBatter
                    HomeUsedPlayers.append(NewBatter)
            ...
            if TeamHitting == 'Away':
                if PitchCount[1] > (HomeLineup[9].NP/HomeLineup[9].GP):
                    HomeLineup[9] = Pitchers_list1[HomeRelieverNumber]
                    HomeRelieverNumber += 1
                    PitchCount[1] = 0
            else:
                if PitchCount[0] > (AwayLineup[9].NP/AwayLineup[9].GP):
    ...
    print("Total Runs: ",(Score[0]+Score[1]))
    ...
    if Score[1] > Score[0]:
        HomeTeamWins += 1
    return  AwayTeamWins, HomeTeamWins, TotalScore
##Game initialization
Outs = 0
...
if GameMode == "100Game":
    ...
    HomeTeamWins = 0
    TotalRunsList = []
if GameMode == "AtBat" or GameMode == "Inning":
    NumberGame = 99
    AwayTeamWins = 0
    HomeTeamWins = 0
    TotalRunsList = []
while NumberGame < 100:
    Score = [0,0]
    AwayTeamWins, HomeTeamWins, TotalScore = MainGame(AwayTeamWins, HomeTeamWins, GameMode)
    TotalRunsList.append(TotalScore)
    NumberGame += 1
MedianRuns = statistics.median(TotalRunsList)
if len(TotalRunsList) > 1:
    print("Number of Wins for {0}: ".format(AwayTeam), AwayTeamWins)
    print("Number of Wins for {0}: ".format(HomeTeam), HomeTeamWins)
    print("Median Number of Runs Scored: ", MedianRuns)
...
#     Pitcher = HomePitcher
#     InningStatus = "Top"
#     while CurrentInning < 10 or (CurrentInning >= 10 and ((Score[0] == Score[1] and InningStatus == "Top")or(Score[0]>=Score[1] and InningStatus == "Bottom")) or (CurrentInning == 9.5 and Score[1]< Score[0])):
#     #(CurrentInning >= 10 and Score[0] == Score[1]) or (CurrentInning == 9.5 and Score[0]< Score[1]):
#         TeamHitting, ScoreINN, HitsINN, CurrentPitcher, PitchCountINN, SpotInLineupINN = HalfInning(TeamHitting, LineUp, SpotInLineup, Pitcher, Score, Hits, PitchCount, GameMode)
#         CurrentInning += 0.5
#         if TeamHitting == "Away":
...
#                 continue
#             if TeamHitting == "Away":
#                 print("\n###Top of the {0}###\n{1} {2}\n{3} {4}\n".format(CurrentInning//1,AwayTeam,Score[0],HomeTeam,Score[1]))
#                 print("{0} is on the mound".format(HomeLineup[9]))
#             if TeamHitting == "Home":
#                 print("\n###Bottom of the {0}###\n{1} {2}\n{3} {4}\n".format(CurrentInning//1,AwayTeam,Score[0],HomeTeam,Score[1]))
#                 print("{0} is on the mound".format(AwayLineup[9]))
#         if GameMode == "Inning" or GameMode == "AtBat":
#             if TeamHitting == 'Away':
#                 DueUp = LineUp[SpotInLineup[0]]
#                 if SpotInLineup[0] == 7:
