...
for i in range(0,2):
    ...
    for x in range(len(PlayersToPass)):
        if float(dataframe.iloc[x][8]) != 0 and float(dataframe.iloc[x][6]) != 0 and float(dataframe.iloc[x][5]) != 0:
            ...
            Hit = (float(dataframe.iloc[x][15])) / float(dataframe.iloc[x][5])
...
def AtBat(Pitcher,Batter,Bases,Outs,Runners, Score):
    ...
    HBP = (Batter.HBP/Batter.PA)
    K = (Pitcher.K/Pitcher.TBF)#Adjust formula  (Batter.K/Batter.PA) * (Pitcher.K/Pitcher.TBF) / 0.222
